# ==============================================================================
# Table S8: 21 GBD Regions — ConGD Burden (GBD 2023, 1990 vs 2023)
# ==============================================================================
library(tidyverse)
library(flextable)
library(officer)

cat("--- Table S8: Regional (GBD 2023) ---\n")
...
save_word_path <- file.path(dir_out, "Table_S8_Regional.docx")
...
d1990 <- df_calc %>% filter(year == 1990) %>%
  dplyr::select(location_name, cgd_rate_1990 = cgd_rate, lo_1990 = cgd_rate_lo,
         up_1990 = cgd_rate_up, pmr_1990 = pmr_val, cmnn_1990 = cmnn_rate)
d2023 <- df_calc %>% filter(year == 2023) %>%
  dplyr::select(location_name, cgd_rate_2023 = cgd_rate, lo_2023 = cgd_rate_lo,
...
df_formatted <- df_wide %>%
  ...
  dplyr::select(Region = location_name, ASMR_1990, PMR_1990, ASMR_2023, PMR_2023,
...
ft <- df_formatted %>%
  ...
  set_caption("Table S8. Regional ConGD mortality burden (1990\u20132023).") %>%
