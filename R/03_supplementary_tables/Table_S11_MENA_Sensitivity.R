...
# Join on location_id to avoid the GBD SDI file's subnational homonyms.
...
top_main <- df_final %>% arrange(desc(gap_main)) %>% head(10) %>% dplyr::select(location_name, sdi_group, gap_main, is_mena)
top_nm   <- df_final %>% filter(!is_mena) %>% arrange(desc(gap_nm)) %>% head(10) %>% dplyr::select(location_name, sdi_group, gap_nm)
...
ft <- ft_df %>% flextable() %>%
  ...
  set_caption("Table S11. Sensitivity analysis excluding high-consanguinity (MENA + Pakistan + Afghanistan) countries from the frontier.") %>%
...
save_as_docx(ft, path = file.path(output_dir, "Table_S11_MENA_Sensitivity.docx"),
             pr_section = prop_section(page_size = page_size(orient = "landscape")))

cat("\nSaved: Table_S11_MENA_Sensitivity.docx\n")
