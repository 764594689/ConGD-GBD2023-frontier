...
# (GBD data quality rating ≥ 4 stars, per the GBD 2019 data-quality appendix)
...
# --- Load GBD stars (country name + star rating) ---
stars <- read_csv(path_stars, show_col_types = FALSE)

# Harmonize the stars file's country names to GBD canonical location_name.
name_map <- c(
  "UK" = "United Kingdom",
...
high_q_names <- stars %>% filter(stars >= 4) %>% pull(location_name)
cat(sprintf("High-quality VR (≥ 4 stars): %d names\n", length(high_q_names)))

# --- Country burden ---
df_raw <- read_csv(path_country, show_col_types = FALSE)

# Country-level id<->name map (for resolving stars$location_name to location_id).
country_id_map <- df_raw %>%
  distinct(location_id, location_name)

high_q <- country_id_map %>% filter(location_name %in% high_q_names) %>%
  pull(location_id)
cat(sprintf("High-quality VR resolved to GBD location_id: %d countries\n",
            length(high_q)))
...
# subnational homonyms (e.g. country Georgia id=35 vs US-state Georgia id=533).
...
df_final <- df_rate %>%
  ...
  mutate(high_quality_vr = location_id %in% high_q)
...
ft <- ft_df %>% flextable() %>%
  ...
  set_caption("Table S10. Sensitivity of avoidable ConGD deaths estimates to frontier specification.") %>%
  add_footer_lines(values = as_paragraph(
    ...
    " countries with GBD data quality rating \u2265 4 stars, then applied to all countries to recompute efficiency gaps. Source of star ratings: the GBD 2019 data-quality appendix."
  ))

save_as_docx(ft, path = file.path(output_dir, "Table_S10_HighQualityVR.docx"),
             pr_section = prop_section(page_size = page_size(orient = "landscape")))

# Save country-level sensitivity CSV
write_csv(df_final %>%
            dplyr::select(location_name, sdi_value, sdi_group, high_quality_vr,
                   total_rate, total_deaths,
                   frontier_main, gap_main, avoid_main,
                   frontier_hq,   gap_hq,   avoid_hq) %>%
            arrange(desc(avoid_hq)),
          file.path(output_dir, "Table_S10_HighQualityVR_country.csv"))

cat("\nSaved:\n  Table_S10_HighQualityVR.docx\n  Table_S10_HighQualityVR_country.csv\n")
