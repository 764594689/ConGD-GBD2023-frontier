# ==============================================================================
# Supplementary Table S9: 204-Country ConGD Mortality Burden (2023)
...
cat("--- Table S9: Country-Level Data (GBD 2023) ---\n")
...
df_all <- df_base %>%
  ...
  dplyr::select(location_name, all_deaths = val)
...
df_display <- df_country %>%
  ...
  dplyr::select(Country = location_name, `ASMR (95% UI)`, `Deaths (N)`, `PMR (%)`)

# --- Save CSV ---
write_csv(df_country, file.path(output_dir, "Table_S9_Country_Data.csv"))
...
ft_s2 <- df_display %>%
  ...
  set_caption(paste0("Table S9. Country-level congenital and genetic disorder mortality burden ",
...
save_as_docx(ft_s2, path = file.path(output_dir, "Table_S9_Country_Data.docx"),
             pr_section = prop_section(
               page_size = page_size(orient = "portrait"),
               page_margins = page_mar(bottom = 0.8, top = 0.8, right = 0.6, left = 0.6)
             ))

cat(sprintf("Table S9 saved. %d countries.\n", nrow(df_display)))
